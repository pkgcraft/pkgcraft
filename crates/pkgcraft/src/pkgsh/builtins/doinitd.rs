...
pub(crate) fn run(args: &[&str]) -> scallop::Result<ExecStatus> {
    ...
    let dest = "/etc/init.d";
    let opts = match build.eapi().has(Feature::ConsistentFileOpts) {
        true => vec!["-m0755"],
        false => build.exeopts.iter().map(|s| s.as_str()).collect(),
    };
...
make_builtin!("doinitd", doinitd_builtin, run, LONG_DOC, USAGE, &[("..", &["src_install"])]);
