...
mod tests {
    ...
    fn test_repo_set_iter_and_contains() {
        ...
        assert!(empty_set.iter_restrict(cpv.clone()).next().is_none());
        ...
        assert!(repo.iter_restrict(cpv.clone()).next().is_none());
        ...
        assert!(repo.iter_restrict(cpv.clone()).next().is_some());
        assert!(repo.contains(&cpv));
        assert!(repo.contains(cpv.clone()));

        // multiple pkgs of different types
        let fake_repo = fake::Repo::new("fake", 0, ["cat/pkg-2"]).unwrap();
        let f_repo: Repo = fake_repo.into();
